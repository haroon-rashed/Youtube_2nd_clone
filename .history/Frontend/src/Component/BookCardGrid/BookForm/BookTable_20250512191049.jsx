...
const BookTable = ({ books, onEdit, onDelete, setGoToTable }) => {
    ...
    <div className="p-6">
      ...
      <div className='flex justify-evenly items-center m-5'>
        {Array.from({ length : totalPages}, (_,i) =>(
          <button key={i + 1} onClick={()=>setCurrentPage(i+1)} style={{padding: '10px', marginTop: '20px'}} className={`p-5 rounded-md ${currentPage === i+ 1 ? 'bg-blue-600 text-white' : "bg-gray-300 text-black"} hover:bg-blue-400` }>{i+1} Page</button>
        ))}
      </div>

      <div className='flex justify-center p-3'>
       <button onClick={()=>setGoToTable(false)} style={{padding: '10px', margin:'10px'}} className='bg-green-500 text-white my-3 rounded-md p-3'> Go Back To Tables Page</button>
      </div>
    </div>
  );
};
