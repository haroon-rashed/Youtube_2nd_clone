    ...
    <div className="p-6">
      ...
      <div className='flex justify-evenly items-center m-5'>
        {Array.from({ length : totalPages}, (_,i) =>(
          <button key={i + 1} onClick={()=>setCurrentPage(i+1)} style={{padding: '10px'}} className={`p-5 rounded-md ${currentPage === i+ 1 ? 'bg-blue-600 text-white' : "bg-gray-300 text-black"} hover:bg-blue-400` }>{i+1} Page</button>
        ))}
      </div>
    </div>
  );
